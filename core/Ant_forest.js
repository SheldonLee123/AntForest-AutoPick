...
function Ant_forest(automator, unlock) {
  ...
  const _collect_friend = function() {
    log("开始收集好友能量");
    if (descEndsWith('View more friends').exists()) {
      descEndsWith('View more friends')
        .findOne(_config.get("timeout_findOne"))
        .click()
    } else if (textEndsWith('View more friends').exists()) {
      textEndsWith('View more friends')
        .findOne(_config.get("timeout_findOne"))
        .click()
    }
    sleep(200)
  ...
  const friendListWaiting = function () {
    return widgetWaiting('Green heroes', '好友排行榜')
  }
  return {
    exec: function() {
      let thread = threads.start(function() {
